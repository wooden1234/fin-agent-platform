...
from app.services.financial.schemas import FinancialQueryIntent, FinancialSqlResultRow
from app.services.financial.sql_executor import FinancialSqlExecutor
...
class FinancialFactService:
    ...
    def resolve_metric_terms(metric: str) -> list[str]:
        return FinancialFactSearchExecutor.resolve_metric_terms(metric)
    ...
    def match_template(
        ...
        """兼容旧调用方；新代码优先使用 FinancialQueryRouter。"""
    ...
    async def execute_query(
        ...
        *,
        ...
        # 路由顺序是：先尝试命中显式模板；没有合适模板时再退回通用结构化搜索。
    ...
    async def run_template(
        ...
        *,
        ...
        return await FinancialTemplateExecutor.run_template(
            ...
            fact_year_sort_key_desc=lambda fact: cls._fact_year_sort_key(
                fact,
                desc=True,
            ),
            fact_year_sort_key_asc=lambda fact: cls._fact_year_sort_key(
                fact,
                desc=False,
            ),
            limit=limit,
    ...
    async def run_sql_template(
        ...
        *,
        limit: int = 5,
    ) -> tuple[list[FinancialSqlResultRow], str, dict[str, Any], list[str]]:
        ...
        rows = await FinancialSqlExecutor.execute(
            built.sql,
            params=built.params,
            limit=limit,
        )
    ...
    async def _search_base(
        ...
        *,
        ...
        """兼容旧调用方；新代码优先使用 FinancialFactSearchExecutor。"""
        return await FinancialFactSearchExecutor.search(
            query,
            limit=limit,
    ...
    def format_answer(cls, facts: list[AnnualFinancialFact]) -> str:
        ...
        # 这里刻意保持“一行一个事实”，让上游 summarize 或 answer agent 再做更复杂的组织。
        lines: list[str] = []
        for fact in facts:
    ...
    @classmethod
    def format_sql_answer(cls, rows: list[FinancialSqlResultRow]) -> str:
        if not rows:
            return "（数据库中未找到匹配的财务指标，建议改查 PDF 文档库。）"
