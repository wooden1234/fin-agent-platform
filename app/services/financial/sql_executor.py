...
from app.services.financial.schemas import FinancialSqlResultRow
...
class FinancialSqlExecutor:
    ...
    @classmethod
    def validate_readonly_sql(cls, sql: str) -> str:
        """校验 SQL 仅包含单条只读查询。"""
        ...
        unknown_tables = {
            table for table in tables if table not in cls.ALLOWED_TABLES
        }
        if unknown_tables:
            raise SqlValidationError(
                f"SQL 使用了非白名单表: {', '.join(sorted(unknown_tables))}"
            )
    ...
    async def execute(
        ...
        *,
        ...
        """执行通过白名单校验的 SQL，并转换成统一结果行。"""
        validated_sql = cls.validate_readonly_sql(sql)
