...
from app.agents.components.finance_agent.pdf_agent.prompts import PDF_BUSY_ANSWER, PDF_NO_CONTEXT_ANSWER, PDF_SYSTEM_PROMPT
from app.agents.states import Citation, FinAgentState
from app.core.config import settings
from app.core.logger import get_logger
from app.retrieval import RetrievalHit, get_pdf_retriever
from app.retrieval.filters import infer_pdf_metadata_filters
...
def _hits_to_citations(hits: list[RetrievalHit]) -> list[Citation]:
    ...
    for h in hits:
        citation: Citation = {"source": h.metadata.get("source", ""), "snippet": (h.text or "")[:200]}
...
async def pdf_agent(
    ...
    if not hits or hits[0].score < min_score:
        ...
        return {
            ...
            "citations": citations,
            "task_results": [{"sub_task_id": sub_task_id, "question": query, "type": "pdf", "context": "（未找到相关文档条目）"}],
            "steps": ["pdf_agent"],
        }
