...
from app.agents.components.finance_agent.financial_query_agent.predefined.extraction.models import (
    PredefinedSlotExtraction,
)
from app.agents.components.finance_agent.financial_query_agent.predefined.extraction.normalizer import (
...
from app.agents.components.finance_agent.financial_query_agent.predefined.whitelist.descriptions import (
    REQUIRED_FIELDS,
    VALID_TEMPLATE_IDS,
    template_catalog_text,
)
from app.agents.components.finance_agent.financial_query_agent.predefined.whitelist.sql_dict import (
    PREDEFINED_SQL_DICT,
)
...
_BOUND_PARAM_RE = re.compile(r":([a-zA-Z_][a-zA-Z0-9_]*)")
...
class PredefinedTemplateRegistry:
    ...
    @classmethod
    def get_sql(cls, template_id: str) -> str | None:
        definition = PREDEFINED_SQL_DICT.get(template_id)
        return definition.sql.strip() if definition else None
    ...
    async def resolve_query(
        ...
        """先查字典完成标准化，再输出执行阶段可直接消费的解析结果。"""
        intent = build_predefined_query_intent(slots)
        return await cls.resolve_intent(template_id, intent)
    ...
    def build_from_resolution(
        ...
        *,
        ...
        from app.agents.components.finance_agent.financial_query_agent.predefined.sql_builder import (
    ...
    @staticmethod
    def _filter_sql_params(sql: str, params: dict[str, object]) -> dict[str, object]:
        """只保留模板 SQL 实际声明的参数，避免向 SQLAlchemy 传递多余绑定值。"""
        declared_params = set(_BOUND_PARAM_RE.findall(sql))
        return {key: value for key, value in params.items() if key in declared_params}
