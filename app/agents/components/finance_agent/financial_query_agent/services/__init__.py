...
from app.agents.components.finance_agent.financial_query_agent.services.entity_resolver import EntityResolver
from app.agents.components.finance_agent.financial_query_agent.services.query_router import (
...
def __getattr__(name: str):
    if name in {"FinancialFactSearchExecutor"}:
        from app.agents.components.finance_agent.financial_query_agent.services.fact_search_executor import (
            FinancialFactSearchExecutor,
        )
    ...
    if name in {"FinancialSqlExecutor", "SqlValidationError"}:
        from app.agents.components.finance_agent.financial_query_agent.services.sql_executor import (
            FinancialSqlExecutor,
            SqlValidationError,
        )
    ...
    if name == "FinancialSqlTemplateRegistry":
        from app.agents.components.finance_agent.financial_query_agent.services.sql_templates import (
            FinancialSqlTemplateRegistry,
        )

        return FinancialSqlTemplateRegistry
    if name == "FinancialTemplateExecutor":
        from app.agents.components.finance_agent.financial_query_agent.services.template_executor import (
            FinancialTemplateExecutor,
        )
