...
from app.agents.components.finance_agent.financial_query_agent.retrievers.sql_examples.base import (
...
LIMIT :limit
""",
                notes=["对比查询保留 period_year 方便上游汇总", "年份条件使用 IN :years"],
            ),
        ),
        "趋势分析": (
            SqlExample(
...
LIMIT :limit
""",
                notes=["趋势查询按年份升序输出", "可结合 years 传入近 N 年集合"],
            ),
        ),
        "多公司对比": (
            SqlExample(
...
LIMIT :limit
""",
                notes=["多公司比较使用 IN :company_names", "排序优先公司名再按年份"],
            ),
        ),
        "排名查询": (
            SqlExample(
...
LIMIT :limit
""",
                notes=["排名查询优先按数值列排序", "如果使用 value 排序，仍需输出展示别名"],
            ),
        ),
    }
